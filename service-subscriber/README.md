...
- **DB Backend Selector (`Database`)**: An object with a `def backend(implicit system: ActorSystem)` that reads `DB_BACKEND` (default: `"timescaledb"`, also accepting `"mysql"`, `"influxdb"` and `"sqlite"`) and instantiates the matching backend. `def readTarget` resolves the read side from the same variable and fails the same way, so a backend name is either supported for both paths or for neither. The `ActorSystem` implicit is required by backends that create child actors. The instance is passed to every `TopicActor` at construction time.
...
- `DB_BACKEND`: Backend implementation, `timescaledb` (default), `mysql`, `influxdb` or `sqlite`
- `DB_HOST`: Database host (default: `timescaledb`; `mysql` or `influxdb` under those backends)
- `DB_PORT`: Database port (default: `5432`; `3306` MySQL, `8086` InfluxDB)
