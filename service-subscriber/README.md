...
- **DB Backend Selector (`Database`)**: An object with a `def backend(implicit system: ActorSystem)` that reads `DB_BACKEND` (default: `"timescaledb"`, also accepting `"mysql"`, `"influxdb"`, `"sqlite"` and `"mongodb"`) and instantiates the matching backend. `def readTarget` resolves the read side from the same variable and fails the same way, so a backend name is either supported for both paths or for neither. The `ActorSystem` implicit is required by backends that create child actors. The instance is passed to every `TopicActor` at construction time.
- **TimescaleDB Backend (`TimescaleDBBackend`)**: The concrete implementation of `DatabaseBackend` for TimescaleDB. Takes an implicit `ActorSystem`. Its connection comes from `DbConfig` (in `Database.scala`), the single reader of `DB_HOST` / `DB_PORT` / `DB_NAME` / `DB_USER` / `DB_PASSWORD` — the same five keys the Erlang arm's backends read, which is what lets one compose fragment configure both repos. The JDBC URL is assembled from them by `DbConfig.urlFor(driver, params)` rather than supplied whole — the scheme and connection options belong to the backend, since they are properties of its driver — and `DbConfig` is shared with the read target so the two can never point at different databases; the batching factors come from `BatchConfig`, which reads `BATCH_ENABLED`, `BATCH_SIZE`, `BATCH_TIMEOUT_MS` and `DB_POOL_SIZE` once for every backend.
  ...
  - **Status writes**: `insertStatus` shares the data path's connections in both modes — the HikariCP pool in non-batch mode, a round-robin `BatchWriterActor` (via the same counter as data) in batch mode. `DB_POOL_SIZE` is therefore the total database connection count, matching the Erlang arm, where `insert_status` likewise routes over the shared worker pool. A dedicated status pool would add two connections the Erlang side does not have, which distorts the pool sweep most at its smallest value.
...
- **InfluxDB Backend (`InfluxDBBackend`)**: The concrete `DatabaseBackend` for InfluxDB 2.7, and the only one that speaks HTTP rather than JDBC. It adds no dependency: `java.net.http` is in the JDK. Its companion object is the shared HTTP half — the write and query URLs, the line-protocol builders, the readiness probe, and one process-wide `HttpClient` — so the write and read paths cannot diverge, the role `DbConfig.urlFor` plays for the JDBC backends. Three deviations, each forced. The client is shared rather than one per writer, since each carries a selector thread and its own executor; it pins `HTTP_1_1`, because the default attempts an `h2c` upgrade and multiplexing would hand this arm wire concurrency `inets` cannot have; and non-batch concurrency is capped by a `Semaphore(DB_POOL_SIZE)` on `blocking-io-dispatcher` rather than a private executor, so the `fixed-pool-size` coupling in `application.conf` keeps holding. Because HTTP connects lazily, the constructor blocks on a readiness probe, restoring the crash-at-startup a JDBC connect gives for free. Writes are upserts keyed by timestamp, so two readings from one sensor in the same microsecond overwrite silently while both count as committed.
- **InfluxDB batch target (`InfluxBatchTarget`)**: Thin next to the JDBC targets. Line protocol has no fixed arity and there is no statement to prepare, so a flushed buffer is just its points joined by newlines and posted — a short timeout flush costs nothing extra, unlike `MySQLBatchTarget`. It owns no connection, so `close()` releases nothing; the shared client outlives it, and JDK 17's `HttpClient` is not closeable in any case. Mirrors `insert_batch/2` in `db_backend_influxdb.erl`.
- **InfluxDB read target (`InfluxReadTarget`)**: Holds the Flux script, built once with the bucket interpolated, and posts it. `group()` is required for equivalence with the SQL backends' `avg`/`count` — without it Flux aggregates per series and returns one row per device — and `reduce` computes both aggregates in one scan. The response body is read in full so the measured time covers the whole round-trip, as `MySQLReadTarget` drains its `ResultSet`. Byte-identical to `?READ_FLUX` in `db_read_backend_influxdb.erl`; the matching rule is per backend.
- **SQLite Backend (`SQLiteBackend`)**: The concrete `DatabaseBackend` for SQLite, embedded through sqlite-jdbc, so there is no database container. Its companion reads `DB_PATH` and `DB_INIT_DIR` instead of `DbConfig`'s five network keys, and owns `open()`, which applies the mounted `connection.sql` and the idempotent `init.sql` to every connection it opens, and `withWriteLock`, one fair process-wide `ReentrantLock` that every write and every connection's setup holds. The JVM would not deadlock without it, but the Erlang arm would, so both arms take the same lock and wait for it the same way. Non-batch mode does not use HikariCP: sqlite-jdbc caches no prepared statements, so a pooled connection would parse the INSERT on every row where the Erlang worker prepares once. Instead it keeps `DB_POOL_SIZE` `SQLiteBatchTarget` slots in a queue and borrows one per row on `blocking-io-dispatcher`, which caps concurrency the way `getConnection` does for the other backends.
- **SQLite batch target (`SQLiteBatchTarget`)**: One configured connection with the INSERT and status statements prepared. A flush is one `BEGIN IMMEDIATE` transaction of single-row inserts, so one fsync and no fixed arity, rolled back on any failure including a failed `COMMIT`. Not `addBatch`, which has no esqlite equivalent. `writeRow` is the non-batch path's single autocommit insert.
- **SQLite read target (`SQLiteReadTarget`)**: One configured connection and the prepared bounded-window SELECT. `Timestamp` holds epoch microseconds, so the window is computed in that unit. Byte-identical to the Erlang arm's SQLite read query.
- **MongoDB Backend (`MongoDBBackend`)**: The concrete `DatabaseBackend` for MongoDB 8.0, through the synchronous Java driver. Its companion owns what the write and read paths share: the `{w: 1, j: true}` write concern, so an ack means the journal fsync the SQL backends' commits mean, the document builders, and two process-wide clients, one for writes sized to `DB_POOL_SIZE` and one for reads sized to `READ_POOL_SIZE`. Each client opens its whole pool at startup and polls the server over one extra monitoring connection. The driver connects lazily, so the constructor pings the database, restoring the crash-at-startup a JDBC connect gives. Non-batch mode runs `insertOne` on `blocking-io-dispatcher`, capped by the pool's checkout the way HikariCP's `getConnection` caps the JDBC backends. The driver holds a connection for one write at a time, while the Erlang arm's driver pipelines several on each. `Data` is a time-series collection whose time field is a BSON Date, so `Clock.toDateMillis` floors the reading's timestamp to milliseconds.
- **MongoDB batch target (`MongoBatchTarget`)**: Thin, like `InfluxBatchTarget`: it borrows a connection from the shared write client per call, so there is nothing to prepare or own. A flush is one ordered `insertMany`, so one journal commit and no fixed arity. Mirrors `insert_batch/2` in `db_backend_mongodb.erl`.
- **MongoDB read target (`MongoReadTarget`)**: Holds the aggregate pipeline as JSON text, parsed once, and runs it on the shared read client, draining the cursor. `$$NOW` keeps the 5-second window on the server clock, like `NOW(6)`. The window scans every time-series bucket, so read cost may grow with the number of rows written. Byte-identical to the Erlang arm's MongoDB pipeline; the matching rule is per backend.
...
- `DB_HOST`: Database host (default: `timescaledb`; `mysql`, `influxdb` or `mongodb` under those backends)
- `DB_PORT`: Database port (default: `5432`; `3306` MySQL, `8086` InfluxDB, `27017` MongoDB)
- `DB_NAME`: Database name (default: `epu`)
- `DB_USER`: Database user (default: `postgres`; `root` under `DB_BACKEND=mysql` and `DB_BACKEND=mongodb`)
- `DB_PASSWORD`: Database password (default: `postgres`; `mysql` under `DB_BACKEND=mysql`, `mongo` under `DB_BACKEND=mongodb`)
- `DB_ORG` / `DB_TOKEN`: InfluxDB only — organisation and API token (defaults: `epu`, `epu-benchmark-token`). The two keys beyond the five every backend reads
- `DB_PATH` / `DB_INIT_DIR`: SQLite only — the database file and the mounted directory holding `init.sql` and `connection.sql` (defaults: `/var/lib/sqlite/epu.db`, `/sqlite/init`). SQLite reads these instead of the five

All of these are supplied by the selected backend's compose fragment. The JDBC scheme is **not** among
...
- State changes are logged via the actor logger (`log.warning`) and persisted asynchronously to `sensor_status` in the selected database, but only when the status actually changes to avoid unnecessary writes.
