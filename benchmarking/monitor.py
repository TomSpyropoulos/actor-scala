...
LATENCY_STAGES = [
    ...
    # Read load is generated by the subscriber's own readers, not by ingest traffic, so this stage
    # is flat at zero in every scenario that leaves READS_PER_SEC at 0.
    ("db_read_latency_ms", "subscriber_read_latency_milliseconds"),
]
...
PANELS = [
    ...
    # Achieved, not configured: a reader paces to a deadline and only ever has one read in flight,
    # so when the target rate is unreachable this falls below READS_PER_SEC rather than backlogging.
    # This is the value the read group must be plotted against.
...
def render_table(sample):
    ...
    for column in ("time", "msgs/s", "committed/s", "reads/s", "sensors up/down", "cpu (cores)", "mem (MB)"):
    ...
    if sample is not None:
        ...
        top.add_row(
            ...
            fmt(_series_value(panels, "reads_per_second", "value")),
